...
def upgrade():
    ...
    if not _index_exists(connection, "idx_saved_insights_user_status"):
        if _column_exists(connection, "saved_insights", "user_id") and _column_exists(
            connection, "saved_insights", "status"
        ):
            op.create_index(
                "idx_saved_insights_user_status", "saved_insights", ["user_id", "status"]
            )
    ...
    if not _index_exists(connection, "idx_research_requests_user_status"):
        if _column_exists(connection, "research_requests", "user_id") and _column_exists(
            connection, "research_requests", "status"
        ):
            op.create_index(
                "idx_research_requests_user_status", "research_requests", ["user_id", "status"]
            )
