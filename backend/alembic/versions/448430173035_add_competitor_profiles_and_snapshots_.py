...
def upgrade() -> None:
    ...
    op.create_foreign_key(
        None, "custom_analyses", "users", ["user_id"], ["id"], ondelete="RESTRICT"
    )
    op.alter_column("insight_interactions", "insight_id", existing_type=sa.UUID(), nullable=True)
    op.execute(
        "ALTER TABLE insight_interactions DROP CONSTRAINT IF EXISTS insight_interactions_insight_id_fkey"
    )
    op.create_foreign_key(
        None, "insight_interactions", "insights", ["insight_id"], ["id"], ondelete="SET NULL"
    )
    op.alter_column(
        "insights",
    ...
    # Use IF NOT EXISTS guards — these columns may already exist if 68bc7f9b5a31 created them
    ...
    op.alter_column("saved_insights", "insight_id", existing_type=sa.UUID(), nullable=True)
    op.execute(
        "ALTER TABLE saved_insights DROP CONSTRAINT IF EXISTS saved_insights_insight_id_fkey"
    )
    op.create_foreign_key(
        None, "saved_insights", "insights", ["insight_id"], ["id"], ondelete="SET NULL"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_shared_insights_shared_by_id ON shared_insights (shared_by_id)"
    )
    op.execute(
        "ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS user_email_snapshot VARCHAR(255)"
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_team_invitations_invited_by_id ON team_invitations (invited_by_id)"
    )
    op.alter_column("user_ratings", "insight_id", existing_type=sa.UUID(), nullable=True)
    op.execute("ALTER TABLE user_ratings DROP CONSTRAINT IF EXISTS user_ratings_insight_id_fkey")
    op.create_foreign_key(
        None, "user_ratings", "insights", ["insight_id"], ["id"], ondelete="SET NULL"
    )
    ...
    op.execute(
        "ALTER TABLE webhook_events DROP CONSTRAINT IF EXISTS uq_webhook_events_stripe_event_id"
    )
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(
        op.f("uq_webhook_events_stripe_event_id"),
        "webhook_events",
        ["stripe_event_id"],
    ...
    op.create_foreign_key(
        op.f("user_ratings_insight_id_fkey"),
        "user_ratings",
        "insights",
        ["insight_id"],
        ["id"],
    ...
    op.alter_column("user_ratings", "insight_id", existing_type=sa.UUID(), nullable=False)
    ...
    op.create_foreign_key(
        op.f("saved_insights_insight_id_fkey"),
        "saved_insights",
        "insights",
        ["insight_id"],
        ["id"],
    ...
    op.alter_column("saved_insights", "insight_id", existing_type=sa.UUID(), nullable=False)
    ...
    op.create_index(
        op.f("idx_research_requests_user_id"), "research_requests", ["user_id"], unique=False
    )
    op.create_index(
        op.f("idx_research_requests_status"), "research_requests", ["status"], unique=False
    )
    op.create_index(
        op.f("idx_research_requests_created_at"),
        "research_requests",
    ...
    op.create_foreign_key(
        op.f("insight_interactions_insight_id_fkey"),
    ...
    op.alter_column("insight_interactions", "insight_id", existing_type=sa.UUID(), nullable=False)
    op.drop_constraint(None, "custom_analyses", type_="foreignkey")
    op.create_foreign_key(
        op.f("custom_analyses_user_id_fkey"),
        "custom_analyses",
        "users",
        ["user_id"],
        ["id"],
