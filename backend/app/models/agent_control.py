...
class AgentConfiguration(Base):
    ...
    cost_limit_daily_usd: Mapped[Decimal] = mapped_column(
    ...
    schedule_type: Mapped[str | None] = mapped_column(
        String(20), nullable=True, doc="cron | interval | manual"
    )
    schedule_cron: Mapped[str | None] = mapped_column(
        String(100), nullable=True, doc="cron expression (e.g., '0 8 * * *')"
    )
    schedule_interval_hours: Mapped[int | None] = mapped_column(
        Integer, nullable=True, doc="interval in hours"
    )
    next_run_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, doc="next scheduled run time"
    )
    last_run_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, doc="last execution time"
    )
