...
class IdeaComment(Base):
    ...
    parent_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("idea_comments.id", ondelete="CASCADE"), nullable=True
    )
    ...
    user: Mapped["User"] = relationship("User", foreign_keys=[user_id], lazy="selectin")
    insight: Mapped["Insight"] = relationship("Insight", foreign_keys=[insight_id], lazy="selectin")
    parent: Mapped["IdeaComment | None"] = relationship(
        "IdeaComment", remote_side=[id], back_populates="replies", lazy="selectin"
    )
    replies: Mapped[list["IdeaComment"]] = relationship(
        "IdeaComment", back_populates="parent", lazy="selectin"
    )
...
class CommentUpvote(Base):
    ...
    comment_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("idea_comments.id", ondelete="CASCADE"), nullable=False
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
    ...
    # Relationships
    user: Mapped["User"] = relationship("User", foreign_keys=[user_id], lazy="selectin")
    comment: Mapped["IdeaComment"] = relationship(
        "IdeaComment", foreign_keys=[comment_id], lazy="selectin"
    )
...
class IdeaPoll(Base):
    ...
    poll_type: Mapped[str] = mapped_column(
        String(20), default="yes_no", nullable=False
    ...
    responses: Mapped[list["PollResponse"]] = relationship(
        "PollResponse", back_populates="poll", cascade="all, delete-orphan", lazy="selectin"
    )
...
class PollResponse(Base):
    ...
    poll_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("idea_polls.id", ondelete="CASCADE"), nullable=False
    )
    user_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False
    )
