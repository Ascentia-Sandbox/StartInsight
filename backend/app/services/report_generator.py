...
"""
    ...
    def generate_markdown_report(self, report: MarketIntelligenceReport) -> str:
        ...
        if report.market_sizing:
            ...
            lines.extend([f"", f"**Confidence Level:** {ms.confidence_level.title()}", f""])
        ...
        if report.industry_benchmarks:
            lines.extend([
                ...
                f"",
                f"| Metric | Industry Avg | Top Performer | Your Target |",
                f"|--------|--------------|---------------|-------------|",
            ])
            for b in report.industry_benchmarks:
        ...
        # Trend opportunities
        if report.trend_opportunities:
            lines.extend([f"## Trend Opportunities", f""])
            for o in report.trend_opportunities:
                lines.extend([
        ...
        # Key insights
        lines.extend([f"## Key Insights", f""])
        for insight in report.key_insights:
            lines.append(f"- {insight}")
        lines.append("")

        # Recommendations
        lines.extend([f"## Strategic Recommendations", f""])
        ...
        lines.extend([
            f"---",
            f"",
            f"*This report was generated by StartInsight Market Intelligence*",
        ])
